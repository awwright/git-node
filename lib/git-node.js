...
function checkoutPackages(err, modulesVersions){
	...
	for(var submodule in modulesVersions) (function(submodule, modulesVersions){
		...
		subgit.getRefs(function(err, refs){
			...
			if(use){
				subgit.checkoutRef(use.ref, function(exit){console.log('\x1b[34m%s\x1b[00m %s %j %d',submodule, use.ref, modulesVersions[submodule], exit);});
			}else{
				console.log('\x1b[34m%s\x1b[00m [none]',submodule);
			}
			//done();
		});
