...
function update(module, modulesVersions, callback){
	...
	relgit.getSubmodulePackages(function(err, list){
		...
		list.forEach(function(submodule){
			...
			relgit.getSubmoduleKey(submodule, 'semver', function(err, semver){
				if(err) throw err;
				if(!semver) return done();
				submodules[submodule] = {module:module, commit:null, submodule:submodule, semver:semver};
				done();
			});
		});
		function done(){
			...
			fs.readFile(path.join(module,'package.json'), function(err, contents){
				...
				if(packageData && packageData.dependencies){
					for(var n in packageData.dependencies){
						...
						if(submodules[submodule]) continue;
						var source = packageData.dependencies[n] || '*';
						if(source.match(/^git/)){
							console.error('Ignoring package.json Git repository '+n+' -> '+source+' (currently unsupported)');
							//submodules[submodule] = {module:module, npmname:n, submodule:submodule, semver:'*'};
						}else{
							submodules[submodule] = {module:module, npmname:n, submodule:submodule, semver:source};
						}
					}
				}
	...
	function updateSubmodule(dependency, cb){
		...
		fs.stat(dependency.submodule, function(err, stat){
...
function parseGitrepos(cb){
	fs.readFile('./.gitrepos', 'utf8', function(err, contents){
		try{
...
function clonePackage(dependency, callback){
	...
	if(dependency.clone){
		...
		// Gitrepos maps an npmname to a Git repository URL
		parseGitrepos(function(err, list){
			if(list && list[dependency.npmname]){
				doClone(list[dependency.npmname]);
				return;
			}
			npm.dereferencePackage(npm.npmURL(dependency.npmname, 'latest'), function(err, packageData){
				...
				if(repo){
					doClone(repo.replace('git@github.com:','git://github.com/'));
				}else{
	...
	function doClone(cloneURL){
		var command = ['clone', cloneURL, dependency.submodule];
		////var clone = (new Git(null)).spawn(['clone','--depth','1',cloneURL,dependency.submodule], {customFds: [-1, process.stdout, process.stderr]});
		var clone = (new Git(null)).spawn(command);
		clone.on('error', function(err){
			console.error('Error running '+command.join('  '));
			console.error(err.stack||err.toString());
		})
		clone.on('close', function(status){
			if(status){
				return callback(new Error('`git '+command.join(' ')+'` exited with status '+status));
			}
...
function setSubmoduleData(dependency, callback){
	...
	if(dependency.clone){
		options.push({key:'submodule.'+submodule+'.url', value:dependency.clone});
	}
