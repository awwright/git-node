			...
			subgit.fetchUpdates(function(exit, v){
				if(exit===127){
					clonePackage(dependency, function(err){
						if(err) throw err;
						update(path.join(module,dependency.submodule), cb);
					});
				}else if(exit){
	...
	update('.', function(){
		...
		for(var submodule in modulesVersions) (function(submodule, modulesVersions){
			...
			subgit.getRefs(function(err, refs){
				...
				var versions = Object.keys(refs).map(parseRefSemver).filter(function(v){return v&&rs.every(function(r){return semver.satisfies(v.semver, r)});}).map(function(v){return v.semver});
				console.log('\x1b[00;33m%s\x1b[00m \t%s', submodule, versions.join('  '));
				//done();
			});
	...
	function tryGitrepos(){
		fs.readFile('./.gitrepos', function(err, contents){
			try{
				...
				return callback(new Error('No repository URL for pacakge '+dependency.npmname));
			}
		});
	}
