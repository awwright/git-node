...
function cmdUpdate(args){
	...
	function update(module, callback){
		...
		function haveDependenciesVersions(submodules){
			...
			updateNextSubmodule(submoduleList, submodules, function(){callback(null);});
		}
	...
	function tryGitrepos(){
		fs.readFile('./.gitrepos', function(err, contents){
			try{
				...
				if(list[dependency.npmname]) doClone(list[dependency.npmname]);
			}catch(err){
				return callback(new Error('No repository URL for package '+dependency.npmname));
			}
		});
	}
