...
function cmdUpdate(args){
	...
	update(args[0]||'.', {}, new Git('.'), checkoutPackages);
}

function checkoutPackages(err, modulesVersions){
	if(err) throw err;
	console.log('Have revisions to install:');
	console.log(modulesVersions);
	for(var submodule in modulesVersions) (function(submodule, modulesVersions){
		...
		subgit.getRefs(function(err, refs){
			if(!refs){ console.error('No refs for '+submodule); return; }
			if(err){ throw err; }
			//console.log('%s %j', submodule, refs);
			//var versions = Object.keys(refs).map(parseRefSemver).filter(function(v){return v&&rs.every(function(r){return semver.satisfies(v.semver, r);})}).map(function(v){return v.semver}).sort(semver.compare);
			var versions = Object.keys(refs).map(parseRefSemver).filter(function(v){return v&&rs.every(function(r){return semver.satisfies(v.semver, r)});});
			console.log('\x1b[00;33m%s\x1b[00m \t%s', submodule, versions.map(function(v){return v.semver}).join(' '));
			var use = versions[versions.length-1];
			if(use){
				subgit.checkoutRef(use.ref, function(exit){console.log('%s %s %d',submodule,use.ref,exit);});
			}
			//done();
		});
...
function update(module, modulesVersions, git, callback){
		...
		subgit.fetchUpdates(function(exit, v){
			...
			if(exit===127){
				clonePackage(dependency, function(err){
					if(err) throw err;
					update(realpath, modulesVersions, git, cb);
				});
			}else if(exit){
	...
	function tryGitrepos(){
		fs.readFile('./.gitrepos', function(err, contents){
			try{
				...
				else throw new Error('No repository URL for package '+dependency.npmname);
			}catch(err){
				return callback(err);
			}
		});
...
function cmdadd(args){
	...
	function postClone(err, dependency){
		...
		var options =
			...
			, {key:'submodule.'+submodule+'.semver', value:dependency.semver}
			];
		function setNext(cb){
		...
		setNext(function addIndex(){
			git.add(['.gitmodules', submodule], done);
		});
	}

	function done(){
		update(packageData.submodule, {}, new Git('.'), checkoutPackages);
	}
}
