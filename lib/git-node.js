...
function cmdadd(args){
	...
	// TODO first we want to poke the remote repository and see:
	//  (1) Which tags are compatible with the requested version, and
	//  (2) If any of them are already checked out locally
	//      If so, symlink to that repository
	//      If not, clone the repository at the most recent tag version

	function cloneFrom(modulename, repositoryURL, checkoutVersion){
		git.fetchRemoteRefs(repositoryURL, function(err, refs){
			var tags = [];
			for(var ref in refs){
	...
	function cloneRepository(modulename, cloneURL, checkoutVersion){
...
function isInstalledGit(commit, cb){
	...
	(new Git('.')).getSubmodulePackages(function(err, modules){
		['.'].concat(modules).forEach(function(module){
			var versions = moduleVersionCache[module] = {};
